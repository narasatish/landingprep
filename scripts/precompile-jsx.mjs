...
let transformSync;
try { ({ transformSync } = await import("esbuild")); } catch (_) { console.log("↷ esbuild not installed — skipping JSX precompilation (pre-built .js files in repo are used)"); process.exit(0); }
