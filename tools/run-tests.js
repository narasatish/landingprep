...
const bundle = exists("app-bundle.js") ? read("app-bundle.js") : "";
...
check("pte-sw-renderer loaded in index.html", /pte-sw-renderer\.jsx?/.test(html) || /pte-sw-renderer\.js/.test(bundle));
...
check("every screen jsx is wired (eager tag or lazy-loaded)", (() => {
  ...
  const miss = screens.filter(f => {
    ...
    return !html.includes("screens/" + f) && !html.includes(js) && !allSrc.includes(js) && !bundle.includes(js);
  });
...
check("TTS audio engine present & loaded", exists("gemini-tts.jsx") && /window\.LP_TTS/.test(read("gemini-tts.jsx")) && (/gemini-tts\.jsx?/.test(html) || /gemini-tts\.js/.test(bundle)));
