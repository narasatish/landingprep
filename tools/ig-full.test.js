...
let hasImage = false;
try { require("@resvg/resvg-js"); hasImage = true; } catch (e) {}
if (!hasImage) { try { require("sharp"); hasImage = true; } catch (e) {} }
...
(async () => {
  ...
  if (!hasImage) {
    console.log("\n[2] Carousel formats — ↷ skip (resvg / sharp not installed)");
  } else {
  ...
  } // end hasImage section [2]

  // ── 3. Image-post slots render ──────────────────────────────────────────────
  if (!hasImage) {
    console.log("\n[3] Image posts — ↷ skip (resvg / sharp not installed)");
  } else {
  ...
  else console.log("  ↷ skip generateStory PNG — resvg / sharp not installed");
  ...
  } else { console.log("  ↷ skip reels — ffmpeg-static not installed"); }
  ...
  else console.log("  ↷ skip carousel idempotency — resvg / sharp not installed");
