...
(async () => {
  ...
  if (!hasImage) { console.log("  ↷ skip — @resvg/resvg-js and sharp not installed"); }
  const topics = new Set(); const covers = new Set();
  for (let d = 0; hasImage && d < 9; d++) {
  ...
  if (hasImage) await test("all 9 distinct carousel formats appeared over 9 consecutive days", () => assert.ok(topics.size >= 8, "only " + topics.size + " distinct topics (expected 9)"));
  else console.log("  ↷ topic-variety check — skip (image deps not installed)");

  // ── 3. Image-post slots render ──────────────────────────────────────────────
  console.log("\n[3] Image posts (slots) render valid PNGs");
  if (!hasImage) { console.log("  ↷ skip — @resvg/resvg-js and sharp not installed"); }
  for (const slot of (hasImage ? [0, 2, 4] : [])) {
  ...
  } else { console.log("  ↷ generateStory PNG — skip (@resvg/resvg-js and sharp not installed)"); }
  ...
  } else { console.log("  ↷ skip — ffmpeg-static not installed"); }
  ...
  } else { console.log("  ↷ carousel idempotency — skip (image deps not installed)"); }
