...
(function () {
  ...
  // ── Prompt library keyed by exam + topic ──────────────────────────────────
  const PROMPTS = {
    ielts: {
      topics: [
  ...
  // Replace `generateAIReply` body with a real API call when backend is ready.
  function generateAIReply(userText, state) {
    ...
    if (/\b(feedback|how did i do|evaluate|my score|am i doing)\b/.test(lower)) {
      return null; // signal: show feedback panel instead
    }

    const acks = [
      "That's a good point.", "Interesting perspective.", "Thank you for sharing that.",
      "I see what you mean.", "That's a thoughtful response.", "Good — let me push you a bit further.",
      "Nicely put.", "That makes sense.",
    ];
    const ack = acks[turnCount % acks.length];
    const nextFU = (followUpList && followUpList.length)
      ? followUpList[followUpIndex % followUpList.length]
      : "What else can you tell me about this? Try to add a reason or an example.";

    // Short answer: rotate between an elaboration nudge and gently moving on, so
    // the examiner NEVER repeats the same line and the conversation keeps flowing.
    if (wordCount < 8) {
      const ELAB = [
      ...
      // Every other short turn, move on to the next question instead of nudging again.
  ...
  function buildFeedback(turns) {
    ...
    // Speaking pace (WPM) from the actual recording time, when available.
    ...
    const wpm = totalMs > 2000 ? Math.round(spokenWords / (totalMs / 60000)) : null;

    // Filler-word detection.
    const lower = " " + allUserText.toLowerCase() + " ";
    let fillerCount = 0;
    FILLERS.forEach((f) => { const m = lower.match(new RegExp("\\b" + f.replace(/ /g, "\\s+") + "\\b", "g")); if (m) fillerCount += m.length; });
    const fillerRate = totalWords > 0 ? Math.round((fillerCount / totalWords) * 1000) / 10 : 0; // per 100 words

    let fluencyScore = Math.min(9, Math.max(3, Math.round(avgLen / 12)));
    // Penalise very slow/fast pace and heavy filler use, reward a natural 120–160 wpm.
    if (wpm != null) { if (wpm < 90 || wpm > 190) fluencyScore = Math.max(3, fluencyScore - 1); else if (wpm >= 110 && wpm <= 170) fluencyScore = Math.min(9, fluencyScore + 1); }
  ...
  // ── Real AI examiner reply (Gemini via backend) ──────────────────────────
  // Falls back to the rule-based generateAIReply when the backend is offline
  // or returns its offline placeholder.
  async function aiExaminerReply(userText, state, turns) {
    ...
    const history = (turns || [])
      ...
      .join("\n");
    const prompt =
      `You are a friendly, professional ${examName} speaking examiner running a live practice ` +
  ...
  function useSpeechSynthesis() {
    ...
    // Native Web Speech fallback (used when Gemini TTS is unavailable).
  ...
  function FeedbackPanel({ fb, onRestart }) {
    if (!fb) return null;
    return (
      React.createElement("div", { className: "feedback-panel" },
        React.createElement("h3", null, "Session Feedback"),
        React.createElement("div", { className: "score-grid" },
          React.createElement("div", { className: "score-cell" },
          ...
          React.createElement("div", { className: "score-cell highlight" },
            React.createElement("div", { className: "score-val" }, fb.overall),
            React.createElement("div", { className: "score-lbl" }, "Overall")
          )
        ),
        React.createElement("p", { style: { color: "#6b7280", fontSize: "0.88rem", margin: "0.5rem 0 0.75rem" } },
          `${fb.turns_u} exchange${fb.turns_u !== 1 ? "s" : ""} · ${fb.totalWords} words spoken`
        ),
        React.createElement("div", { className: "fluency-metrics" },
          React.createElement("div", { className: "fm-item" },
        ...
        React.createElement("ul", { className: "tip-list" },
          fb.tips.map((t, i) => React.createElement("li", { key: i }, t))
        ),
        React.createElement("button", { className: "btn-primary mt-1", onClick: onRestart },
          "Start New Session"
        )
      )
    );
  ...
  function SpeakingAgent({ exam }) {
    ...
    const agentReply = useCallback((text, state) => {
      // Explicit feedback request → show the feedback panel immediately.
      ...
      setAgentTyping(true);
      // Try a real Gemini examiner reply; fall back to the rule-based one if
      // the backend is offline. The rule-based reply is always available so
      ...
      aiExaminerReply(text, state, [...turns, { role: "user", text }])
        .then((aiReply) => advance(aiReply || ruleReply))
        .catch(() => advance(ruleReply));
    ...
    if (phase === "setup") {
      return (
        React.createElement("div", { className: "agent-shell" },
          ...
          React.createElement("div", { className: "setup-card" },
            ...
            React.createElement("div", { className: "setup-opener-preview" },
    ...
    return (
      React.createElement("div", { className: "agent-shell" },
        ...
        React.createElement("div", { className: "chat-area" },
          ...
          agentTyping && React.createElement("div", { className: "chat-bubble chat-agent typing-indicator" },
        ...
        React.createElement("div", { className: "input-bar" },
          micSupported && React.createElement("button", {
            ...
            onClick: () => {
              if (listening) { stopMic(); setAutoMic(false); }
              else { setAutoMic(true); startMic(); }
            },
            title: listening ? "Tap to pause microphone" : autoMic ? "Microphone on — tap to pause" : "Tap to start speaking",
          },
            listening
              ? React.createElement("span", { className: "mic-pulse" }, "●")
              : autoMic ? "🎙" : "🔇"
          ),
        ...
        speaking && React.createElement("div", { className: "agent-speaking-bar" },
