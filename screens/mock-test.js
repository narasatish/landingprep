...
function TestSelect({ exam, onSelect, onBack, onNav }) {
  ...
  const DRILLS = {
    ielts: [
      ...
      { id: "section_speaking", icon: "\u{1F3A4}", name: "Speaking with Examiner", desc: "Part 1 \xB7 Cue Card \xB7 Part 3 discussion", meta: ["14 min", "natural voice", "Scored"] }
    ],
    toefl: [
      ...
      { id: "section_speaking", icon: "\u{1F3A4}", name: "Speaking with Examiner", desc: "4 tasks \xB7 speaking examiner", meta: ["16 min", "4 tasks", "0\u201330"] }
    ],
    gmat: [
    ...
    celpip: [
      ...
      { id: "section_speaking", icon: "\u{1F3A4}", name: "Speaking with Examiner", desc: "8 tasks: advice \xB7 scene \xB7 prediction \xB7 opinion", meta: ["20 min", "8 tasks", "CLB 1\u201312"] }
    ],
    duolingo: [
...
function ListeningQuestions({ questions, answers, setAnswer, sectionId, partIdx, totalParts, formLayout, onPrevPart, onNextPart, isCelpip }) {
  ...
  if (isCelpip) {
    ...
    return /* @__PURE__ */ React.createElement("div", { style: { marginTop: 18 } }, /* @__PURE__ */ React.createElement("div", { className: "celpip-q-progress" }, "Question ", idx + 1, " of ", total), /* @__PURE__ */ React.createElement(
      QuestionCard,
      {
        key: q.id,
        q,
    ...
    ), /* @__PURE__ */ React.createElement("div", { className: "celpip-q-nav" }, /* @__PURE__ */ React.createElement("button", { className: "btn", disabled: idx === 0, onClick: () => setQIdx((i) => Math.max(0, i - 1)) }, "\u2190 Previous"), /* @__PURE__ */ React.createElement("span", { className: "cqn-dots" }, questions.map((qq, i) => /* @__PURE__ */ React.createElement("span", { key: i, className: "cqn-dot" + (i === idx ? " active" : "") + (answers[sectionId + "_" + qq.id] != null && answers[sectionId + "_" + qq.id] !== "" ? " done" : ""), onClick: () => setQIdx(i) }))), idx < total - 1 ? /* @__PURE__ */ React.createElement("button", { className: "btn btn-primary", onClick: () => setQIdx((i) => Math.min(total - 1, i + 1)) }, "Next question \u2192") : /* @__PURE__ */ React.createElement("button", { className: "btn btn-primary", onClick: () => onNextPart && onNextPart() }, "Next part \u2192")));
  }
    ...
    if (isOetRolePlay) {
      ...
      (sec.cards || []).forEach((card, i) => {
        ...
        items.push({
          ...
          roleplay: card.prompt || card.topic || "",
          tip: card.tip || "",
          prep,
          expectSec: card.responseSeconds || 240,
...
Write a 2\u20133 paragraph spoken response (approximately 90\u2013150 words). Use clear, natural spoken English \u2014 contractions are fine. Begin speaking directly without preamble.`;
  ...
  )) : /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8 } }, canGenerate && /* @__PURE__ */ React.createElement("button", { className: "btn btn-sm", onClick: generateModelAnswer, disabled: generating }, generating ? "Generating\u2026" : "\u2728 Generate model answer"), !canGenerate && !modelAnswer && /* @__PURE__ */ React.createElement("div", { className: "review-explanation", style: { color: "var(--ink-3)" } }, "No model answer available. Set a Gemini API key in Speaking & Writing to enable model answers."), error && /* @__PURE__ */ React.createElement("div", { style: { color: "var(--error)", fontSize: 13, marginTop: 6 } }, error)), /* @__PURE__ */ React.createElement("div", { className: "review-explanation" }, /* @__PURE__ */ React.createElement("strong", null, "Rubric:"), " ", (q.rubric || ["fluency", "coherence", "vocabulary", "grammar", "pronunciation"]).join(" \xB7 ")));
}
...
Correct answer: ${correctText}`;
      ...
      if (!clean || /(?:AI|Smart) Tutor is offline/i.test(clean)) {
        setState("error");
        return;
      }
      setText(clean);
...
function ScoreCardButton({ exam, report }) {
  ...
  const draw = () => {
    ...
    ctx.beginPath();
    ctx.arc(1070, 110, 240, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(150, 610, 190, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "rgba(255,255,255,0.10)";
    roundRectPath(ctx, 56, 56, W - 112, H - 112, 32);
    ctx.fill();
    ...
    ctx.fillStyle = "#fff";
    ctx.font = "800 46px system-ui,Segoe UI,Arial";
    ctx.fillText("\u{1F393} LandingPrep", 100, 152);
    ctx.font = "700 30px system-ui,Segoe UI,Arial";
    ctx.fillStyle = accent;
    ctx.fillText((exam.name + "  \xB7  MOCK TEST").toUpperCase(), 102, 250);
    ctx.fillStyle = accent;
    roundRectPath(ctx, 100, 300, 14, 172, 7);
    ctx.fill();
    ctx.fillStyle = "#fff";
    ctx.font = "900 200px system-ui,Segoe UI,Arial";
...
function ShareScoreCard({ exam, report }) {
  ...
  const msg = "I scored " + scoreTxt + " on a free " + examName + " mock test on LandingPrep \u{1F3AF} Practise free for 15+ exams \u2014 IELTS, TOEFL, OET, PTE, GRE & GMAT \u{1F449} " + url;
  const ga = (method) => {
    try {
