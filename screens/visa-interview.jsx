...
// questions; the student answers BY SPEAKING (like the real interview) or typing and
// gets instant feedback (credibility, clarity, red flags). Offline fallback per question.
(function () {
  const { useState, useRef, useEffect } = React;

  // Real, commonly-asked student-visa interview questions per destination, with
  // model guidance. Sourced from official prep guidance + widely-reported interview
  // experiences; every tip is practical, not generic.
  const QUESTIONS = {
    USA: [
  ...
  function VisaInterviewPanel({ country }) {
    ...
    // Speak your answer — the real interview is spoken, so practising aloud matters
    // more than typing. Uses the same SpeechRecognition the mock tests use.
    const startMic = () => {
    ...
    const stopMic = () => { try { recogRef.current && recogRef.current.stop(); } catch (e) {} setListening(false); };
    ...
    const next = () => { stopMic(); setIdx((i) => (i + 1) % list.length); setAnswer(""); setFb(null); };

    return (
      <div className="visa-panel">
        <div className="tool-card">
          <h2>🛂 Visa Interview — Mock &amp; Coach</h2>
          <p className="tool-sub">Practise real student-visa interview questions <strong>out loud</strong> — speak your answer into the mic (or type it) and get instant feedback on credibility, clarity and red flags.</p>
          <div className="tool-row">
            <label>Destination
              <select value={c} onChange={(e) => { stopMic(); setC(e.target.value); setIdx(0); setAnswer(""); setFb(null); }}>{COUNTRIES.map((x) => <option key={x} value={x}>{x}</option>)}</select>
            </label>
        ...
        <div className="tool-card visa-q-card">
          ...
          <textarea className="visa-answer" rows={4} value={answer} onChange={(e) => setAnswer(e.target.value)}
            placeholder={micSupported ? "Tap 🎤 and answer aloud like you would to the officer — or type here…" : "Type your answer as you would say it to the officer…"} />
          <div className="visa-btns">
            {micSupported && (
              <button className={"tool-btn" + (listening ? "" : " ghost")} onClick={listening ? stopMic : startMic}
                style={listening ? { background: "#dc2626", color: "#fff", border: "none" } : undefined}>
                {listening ? "⏹ Stop — I'm done" : "🎤 Answer aloud"}
              </button>
            )}
          ...
          {listening && <p className="visa-tip" style={{ color: "#dc2626", fontWeight: 600 }}>● Listening… speak naturally, then press Stop.</p>}
          {!fb && !listening && <p className="visa-tip">💡 <strong>Tip:</strong> {cur.tip}</p>}
