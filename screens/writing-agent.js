...
In conclusion, while Canada remained the leading renewable electricity producer throughout, Germany's sustained policy-driven growth brought it close to matching Canada's share by 2022.` }
    ...
    toefl: [
      {
        id: "toefl-discussion-1",
        type: "Academic Discussion",
        taskKind: "toefl_discussion",
        target: 100,
...
I would therefore encourage the council to invest in the library project, as its long-term educational and social impact represents the greater benefit to our community as a whole.` }
    ...
    duolingo: [
      ...
      },
      {
        ...
        taskKind: "duolingo",
        target: 35,
        label: "Photo: Caf\xE9 Scene",
        photoUrl: "https://images.unsplash.com/photo-1445116572660-236099ec97a0?w=600&q=80",
        photoAlt: "Two people sitting at a caf\xE9 table, drinking coffee and having a conversation",
        photoFallbackDescription: "The image shows two people seated at a small table inside a caf\xE9. Each person has a cup in front of them. They appear to be engaged in friendly conversation. The caf\xE9 has warm lighting and wooden furniture.",
        ...
        bandSample: { 5: `The image shows two people sitting at a caf\xE9 table and enjoying a conversation over coffee. The caf\xE9 has a warm, cosy atmosphere with soft lighting and wooden furniture. Both people appear relaxed and engaged, suggesting they are friends catching up or colleagues taking a break.` }
      },
      {
        id: "duo-photo-3",
        type: "Write About the Photo",
  ...
  function BuildASentence({ sentences }) {
    ...
    return React.createElement(
      "div",
      { className: "build-sentence-wrap" },
      React.createElement("div", { className: "bs-progress" }, `Sentence ${idx + 1} of ${sentences.length}`),
      React.createElement(
        "div",
        { className: "bs-words" },
        shuffled.map(
          (w, i) => React.createElement("span", { key: i, className: "bs-word-chip" }, w)
        )
      ),
      React.createElement("div", { className: "bs-hint", style: { color: "#6b7280", fontSize: "0.82rem", marginBottom: 8 } }, `\u{1F4A1} Hint: ${item.hint}`),
      ...
      React.createElement(
        ...
        React.createElement("button", { className: "btn-outline", style: { fontSize: "0.85rem" }, onClick: () => {
          setUserAnswer("");
          setChecked(false);
        } }, "Clear"),
        idx < sentences.length - 1 && React.createElement("button", { className: "btn-outline", style: { fontSize: "0.85rem" }, onClick: () => {
          setIdx(idx + 1);
          setUserAnswer("");
          setChecked(false);
        } }, "Next \u2192")
      ...
      checked && React.createElement(
        ...
        isCorrect ? React.createElement("span", { style: { color: "#166534" } }, "\u2713 Correct! Well done.") : React.createElement("span", { style: { color: "#991b1b" } }, `\u2717 Suggested answer: "${item.answer}"`)
      )
    );
  }
  function DuoPhoto({ promptData }) {
    ...
    }, [promptData == null ? void 0 : promptData.id]);
    useEffect(() => {
    ...
    return React.createElement(
      ...
      !imgError ? React.createElement("img", {
        ...
        React.createElement("strong", { style: { display: "block", marginBottom: 4, color: "#4b5563" } }, "\u{1F4F7} Scene Description (image unavailable):"),
  ...
  function ScorePanel({ result, showSample, sampleText, onToggleSample }) {
    ...
    return React.createElement(
      ...
      }, showSample ? "Hide Sample Answer" : "\u{1F4C4} Show Band 7+ Sample Answer"),
      showSample && sampleText && React.createElement(
        "div",
        { className: "sample-answer-block" },
  ...
  function WritingAgent({ exam }) {
    ...
    const minWords = EXAM_MIN_WORDS[currentPrompt == null ? void 0 : currentPrompt.taskKind] || (currentPrompt == null ? void 0 : currentPrompt.target) || 150;
    const sampleKey = Object.keys((currentPrompt == null ? void 0 : currentPrompt.bandSample) || {}).sort((a, b) => b - a)[0];
    const sampleText = (_a = currentPrompt == null ? void 0 : currentPrompt.bandSample) == null ? void 0 : _a[sampleKey];
    const handleScore = () => {
      if (!text.trim()) return;
...
REWRITE: (one of the candidate's weaker sentences, rewritten to a higher band).`;
    ...
    const isBuildSentence = (currentPrompt == null ? void 0 : currentPrompt.taskKind) === "toefl_sentence";
    const isDuoPhoto = (currentPrompt == null ? void 0 : currentPrompt.taskKind) === "duolingo" && (currentPrompt == null ? void 0 : currentPrompt.photoUrl);
    const isToeflEmail = (currentPrompt == null ? void 0 : currentPrompt.taskKind) === "toefl_email";
    const isToeflDiscussion = (currentPrompt == null ? void 0 : currentPrompt.taskKind) === "toefl_discussion";
    return React.createElement(
      "div",
      { className: "writing-agent-shell" },
      // Header
      React.createElement(
        "div",
      ...
      React.createElement(
        ...
        promptList.map(
          (p, i) => React.createElement("button", {
            key: p.id + "_" + i,
          ...
          onClick: async () => {
            try {
              ...
              if (samples && samples.length) {
                const pick = window.LP_HUB.pickRandom(samples, currentPrompt == null ? void 0 : currentPrompt.id);
                if (pick) {
                  const extraPrompt = {
                    id: "hub_" + pick.id,
                    type: "New Prompt",
      ...
      React.createElement(
        ...
        !isDuoPhoto && window.LP_VisualRenderer && (currentPrompt.visual || /\b(chart|graph|table|map|diagram|process)\b/i.test(currentPrompt.prompt || "")) && currentPrompt.taskKind === "ielts_task1" && React.createElement(window.LP_VisualRenderer, { task: { prompt: currentPrompt.prompt, taskType: "academic_task_1_chart", visual: currentPrompt.visual || null } }),
        // TOEFL Discussion professor + student posts
        isToeflDiscussion && currentPrompt.professor && React.createElement(
          "div",
          { className: "toefl-discussion-setup" },
          React.createElement(
            "div",
            { style: { background: "var(--tint-blue)", border: "1px solid #c7d2fe", borderRadius: 8, padding: "12px 14px", marginBottom: 8 } },
          ...
          (currentPrompt.studentPosts || []).map(
            (s, i) => React.createElement(
              "div",
              { key: i, style: { background: "var(--tint-gray)", border: "1px solid #e5e7eb", borderRadius: 8, padding: "10px 14px", marginBottom: 6 } },
        ...
        isToeflEmail && React.createElement(
          "div",
          { style: { background: "var(--tint-amber)", border: "1px solid #fdba74", borderRadius: 8, padding: "12px 14px", marginBottom: 8 } },
          ...
          React.createElement(
            ...
            (currentPrompt.bullets || []).map(
              (b, i) => React.createElement("li", { key: i, style: { fontSize: "0.88rem", lineHeight: 1.65, color: "#374151" } }, b)
            )
          )
        ),
        // Build-a-Sentence task
        isBuildSentence ? React.createElement(BuildASentence, { sentences: currentPrompt.sentences || [] }) : React.createElement(
          React.Fragment,
          null,
          ...
          React.createElement(
            ...
            React.createElement("span", { style: { color: "#6b7280" } }, " \xB7 "),
            React.createElement("span", null, currentPrompt.label)
          )
        )
      ),
      // Tab bar (only for non-Build-a-Sentence tasks)
      !isBuildSentence && React.createElement(
        "div",
        { className: "wa-tab-bar" },
        React.createElement("button", {
      ...
      // Write tab
      !isBuildSentence && tab === "write" && React.createElement(
        "div",
        { className: "write-tab" },
        ...
        // Word count bar
        minWords > 0 && React.createElement(
          "div",
          ...
          React.createElement(
            "div",
            { className: "wc-label", style: { color: wcColor } },
        ...
        aiFb && React.createElement(
          "div",
          { className: "ai-feedback-panel" },
          React.createElement("div", { className: "ai-fb-head" }, "\u2728 Examiner Feedback"),
          aiFb.offline ? React.createElement("p", { className: "ai-fb-offline" }, "The AI examiner is offline right now (start the AI backend to enable it). Your instant heuristic score and the Band model answer below are still available.") : React.createElement("div", { className: "ai-fb-body" }, aiFb.text)
        ),
      ...
      !isBuildSentence && tab === "chat" && React.createElement(ChatPanel, {
        ...
        examId: EXAM_ID,
        currentPromptLabel: currentPrompt == null ? void 0 : currentPrompt.label
      })
    );
  }
