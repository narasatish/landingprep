...
Career goal: ${f.career || "(unspecified)"}`;
      try {
        if (window.LP_AI_TUTOR && window.LP_AI_TUTOR.generate) {
          ...
          if (r && !/(?:AI|Smart) Tutor is offline/i.test(r)) {
            setOut(r);
            setSrc("ai");
            setBusy(false);
            return;
